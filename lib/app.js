...
// ---------- 语音识别（百度语音 REST）----------
// 浏览器自带语音识别在国内连不上 Google，故走百度：前端录音转 16k 单声道 wav → 这里调百度转文字。
let _baiduToken = { val: null, exp: 0 };
async function baiduToken() {
  if (_baiduToken.val && Date.now() < _baiduToken.exp) return _baiduToken.val;
  const key = process.env.BAIDU_API_KEY, secret = process.env.BAIDU_SECRET_KEY;
  if (!key || !secret) throw new Error('服务端未配置百度语音密钥（BAIDU_API_KEY / BAIDU_SECRET_KEY）');
  const r = await fetch(`https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=${encodeURIComponent(key)}&client_secret=${encodeURIComponent(secret)}`, { method: 'POST' });
  const j = await r.json();
  if (!j.access_token) throw new Error('百度令牌获取失败：' + (j.error_description || j.error || '未知'));
  _baiduToken = { val: j.access_token, exp: Date.now() + Math.max(60, (j.expires_in || 2592000) - 3600) * 1000 };
...
app.post('/api/voice', auth, async (req, res) => {
  try {
    ...
    if (!audio) return err(res, 400, '缺少音频数据');
    const token = await baiduToken();
    const len = Buffer.from(audio, 'base64').length;
    const r = await fetch('https://vop.baidu.com/server_api', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        format: format || 'wav', rate: rate || 16000, channel: 1,
        cuid: 'family-asset', token, dev_pid: 1537, // 1537=普通话(含简单标点)
        speech: audio, len,
      }),
    });
    const j = await r.json();
    if (j.err_no !== 0) return err(res, 400, '识别失败：' + (j.err_msg || j.err_no));
    const text = (j.result && j.result[0]) || '';
    let ops = null;
    try { ops = await llmParse(text, data.items.filter((i) => i.familyId === req.user.familyId)); }
    catch { ops = null; }
    ok(res, { text, ops });
  } catch (e) {
    err(res, 500, e.message || '语音识别服务出错');
  }
});
